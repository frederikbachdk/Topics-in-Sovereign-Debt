...
for(i in 1:length(ctry_tbls)){
  ...
  model <- lm(spread_change ~ lag(spread_change) + reer_change + vix_change, ctry) 
  fit <- summary(model)
...
for(i in 1:length(ctry_tbls)){
  ctry <- ctry_tbls[i]
  ctry <- ctry[[1]] %>% filter(X <= 79)
  model <- lm(spread_change ~ lag(spread_change) + reer_change + vix_change + 
                      bidask_change + ind_growth_change + exp_debtgdp_change + exp_govbal_change, ctry)
...
for(i in 1:length(ctry_tbls)){
  ...
  model <- lm(spread_change ~ lag(spread_change) + reer_change + vix_change + PC2_S2_change, ctry)
  fit <- summary(model)
  t_vals <- coeftest(model, vcov = vcovHC(model, type = "HC0"))[,3]
...
for(i in 1:length(ctry_tbls)){
  ...
  ctry <- ctry[[1]] %>% filter(X <= 144) %>% filter(X > 79)
  model <- lm(spread_change ~ lag(spread_change) + reer_change + vix_change + 
                      bidask_change + ind_growth_change + exp_debtgdp_change + 
                      exp_govbal_change + PC2_S2_change, ctry)
  fit <- summary(model)
  t_vals <- coeftest(model, vcov = vcovHC(model, type = "HC0"))[,3]
...
for(i in 1:length(ctry_tbls)){
  ...
  model <- lm(spread_change ~ lag(spread_change) + reer_change + vix_change + 
                bidask_change + ind_growth_change + exp_debtgdp_change + 
                exp_govbal_change + PC2_S3_change, ctry)
  fit <- summary(model)
  t_vals <- coeftest(model, vcov = vcovHC(model, type = "HC0"))[,3]
...
write.xlsx(reg_m1s1_table, file="Estimation/est_resultsFD.xlsx", 
           sheetName="m1s1")

write.xlsx(reg_m2s1_table, file="Estimation/est_resultsFD.xlsx", 
           sheetName="m2s1", append = TRUE)

write.xlsx(reg_m1s2_table, file="Estimation/est_resultsFD.xlsx", 
           sheetName="m1s2", append = TRUE)

write.xlsx(reg_m2s2_table, file="Estimation/est_resultsFD.xlsx", 
           sheetName="m2s2", append = TRUE)

write.xlsx(reg_m1s3_table, file="Estimation/est_resultsFD.xlsx", 
           sheetName="m1s3", append = TRUE)

write.xlsx(reg_m2s3_table, file="Estimation/est_resultsFD.xlsx", 
