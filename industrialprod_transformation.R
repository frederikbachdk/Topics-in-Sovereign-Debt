...
path <- "industrialprod.xlsx"
data <- read_xlsx(path) 
colnames(data) <- c("country", "date", "ind_growth")

data <- data %>% mutate(date = 
                  paste0(substr(date,6,7),"-",substr(date,1,4)))

data_tib <- data %>% 
  pivot_wider(id_cols = "date", names_from = "country", values_from = "ind_growth") %>%
  mutate(date = 
           case_when(
